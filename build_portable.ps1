# Build the Windows portable zip: official CPython + wheels + ffmpeg + yolo + .bat
...
$Stage = Join-Path $Dist "SkateClipCutter"
...
Copy-Item -LiteralPath (Join-Path $PSScriptRoot "assets") -Destination (Join-Path $Stage "assets") -Recurse -Force

$models = Join-Path $Stage "models"
New-Item -ItemType Directory -Force -Path $models | Out-Null
$srcModel = Join-Path $PSScriptRoot "yolo11n.pt"
$altModel = Join-Path $PSScriptRoot "models\yolo11n.pt"
if (Test-Path $srcModel) {
  ...
  throw "yolo11n.pt not found. Run the worker once locally or place the file in the repo root."
}

Write-Host "Copying FFmpeg..."
$ffDir = Join-Path $Stage "vendor\ffmpeg"
...
& $py -c "import tkinter, ultralytics, cv2, torch; from pathlib import Path; print('tk', tkinter.TkVersion); print('torch', torch.__version__); print('ok')"
if ($LASTEXITCODE -ne 0) { throw "Portable Python smoke failed." }

$ffSmoke = Join-Path $ffDir "ffmpeg.exe"
& $ffSmoke -version | Select-Object -First 1

Write-Host "Zipping $ZipName ..."
if (Test-Path $ZipPath) { Remove-Item $ZipPath -Force }
Push-Location $Dist
tar -a -c -f $ZipName SkateClipCutter
Pop-Location
